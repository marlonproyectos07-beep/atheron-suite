...
var esClaveDe = (catalogo, bruto) => typeof bruto === "string" && Object.hasOwn(catalogo, bruto);
...
function caracterControl(aliado, cola2) {
  let suma = semilla(aliado);
  for (let i = 0; i < cola2.length; i++) {
    const valor = ALFABETO.indexOf(cola2[i]);
    if (valor < 0) throw new Error(`Caracter fuera del alfabeto: ${cola2[i]}`);
...
var esFuente = (v) => esClaveDe(ETIQUETA_FUENTE, v);
var ORIGINADOR_LA_TRIADA = "Josu\xE9";
var MAX_PERSONAS = 60;
function creaActivacion(datos) {
  const ahora = datos.ahora ?? /* @__PURE__ */ new Date();
  const sello = selloColombiano(ahora);
  const consiente = datos.consentimiento?.seguimiento === true;
  const contacto = consiente ? normalizaContacto(datos.consentimiento?.contacto) : void 0;
...
function normalizaContacto(bruto) {
  const leido = texto(bruto, 25);
  if (!leido.valor) return void 0;
  const digitos = leido.valor.replace(/\D+/g, "");
  if (digitos.length < 7 || digitos.length > 15) return void 0;
...
async function activar(entrada, deposito = almacen()) {
  for (let intento = 0; intento < 5; intento++) {
    const transaccion = creaActivacion({
      ...
      consentimiento: entrada.consienteSeguimiento ? { seguimiento: true, contacto: String(entrada.contacto ?? "") } : void 0
    });
  ...
  return { ok: false, motivo: "ERROR", explicacion: "No se pudo generar un c\xF3digo libre." };
}
...
// servidor/activar.ts
var activar_default = maneja("POST", async (peticion) => {
  ...
  if (datos.personas !== void 0 && datos.personas !== null) {
    if (entero(datos.personas, 1, MAX_PERSONAS).valor === void 0) {
      return {
        estado: 400,
  ...
  if (datos.contacto !== void 0 && datos.contacto !== null && typeof datos.contacto !== "string") {
