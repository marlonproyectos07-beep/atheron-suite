...
function entero(bruto, min, max) {
  if (bruto === void 0 || bruto === null) return AUSENTE;
...
var pesosEnteros = (bruto, max) => entero(bruto, 1, max);
function texto(bruto, max) {
  if (bruto === void 0 || bruto === null) return AUSENTE;
  if (typeof bruto !== "string") return { fallo: "TIPO" };
  const limpio = bruto.trim().slice(0, max);
  return limpio ? { valor: limpio } : AUSENTE;
}
function objetoPlano(bruto) {
  if (!bruto || typeof bruto !== "object" || Array.isArray(bruto)) return {};
  const limpio = /* @__PURE__ */ Object.create(null);
  for (const [clave2, valor] of Object.entries(bruto)) {
    if (clave2 === "__proto__" || clave2 === "constructor" || clave2 === "prototype") continue;
    limpio[clave2] = valor;
  }
  return limpio;
}
...
function leerCodigo(texto2, aliadoEsperado) {
  ...
  const [, aliado, cuerpo2] = encaja;
  const bonito = `ATH-${aliado}-${cuerpo2}`;
  ...
  const azar = cuerpo2.slice(0, AZAR);
  const control = cuerpo2[AZAR];
  for (const c of cuerpo2) {
...
function selloColombiano(instante) {
...
comprueba2(REGLA);
var instantanea = (regla = REGLA) => ({
  version: regla.version,
  comisionPct: regla.comisionPct,
  creditoPct: regla.creditoPct,
  margenPct: regla.margenPct
});
function calculaEconomia(consumo, regla = REGLA) {
  if (!Number.isFinite(consumo) || consumo < 0) throw new Error("Consumo no v\xE1lido.");
  if (Math.round((regla.creditoPct + regla.margenPct) * 100) !== Math.round(regla.comisionPct * 100)) {
    throw new Error(`ECONOMIA: la regla ${regla.version} no cuadra: cr\xE9dito + margen \u2260 comisi\xF3n.`);
  }
...
var AMBITOS_PREVISTOS = ["HOSPEDAJE", "SECURITY", "EXPERIENCIAS", "ALIADOS"];
...
var ALFABETO2 = "ABCDEFGHJKMNPQRSTUVWXYZ23456789";
function nuevoIdCredito() {
  ...
  return `ATH-CR-${Array.from(n, (v) => ALFABETO2[v % ALFABETO2.length]).join("")}`;
}
function generaCredito(entrada) {
  if (!Number.isInteger(entrada.valor) || entrada.valor <= 0) {
    throw new Error("Un cr\xE9dito de cero o con decimales no se emite.");
  }
  const ahora = entrada.momentoComercial;
  if (!(ahora instanceof Date) || Number.isNaN(ahora.getTime())) {
    throw new Error("Un cr\xE9dito sin momento comercial no se emite: su vigencia ser\xEDa inventada.");
  }
  const expira = new Date(ahora.getTime() + VIGENCIA_CREDITO_DIAS * 24 * 60 * 60 * 1e3);
  return {
...
var porFallo = (fallo, invalido) => fallo ? invalido : invalido;
function redime(transaccion, consumoBruto, opciones = {}) {
...
function cierraSinConsumo(transaccion, ahora = /* @__PURE__ */ new Date()) {
  if (transaccion.estado === "REDIMIDO") return { ok: false, motivo: "YA_REDIMIDA", transaccion };
  if (transaccion.estado === "NO_REDIMIDO") return { ok: false, motivo: "CERRADA", transaccion };
  const sello = selloColombiano(ahora);
  return {
...
async function avisa(evento, datos) {
  ...
  try {
    await fetch(destino, {
      ...
      body: JSON.stringify({ evento, piloto: PILOTO.id, sello: (/* @__PURE__ */ new Date()).toISOString(), ...datos })
    });
  } catch {
  }
}
async function emiteCredito(transaccion, deposito) {
  ...
  try {
    ...
    registra("emitir el cr\xE9dito", error);
    return false;
  }
}
async function redimir(entrada, deposito = almacen(), ahora = /* @__PURE__ */ new Date()) {
  const guardada = await deposito.lee(TX, entrada.codigo);
  if (!guardada) return rechaza("NO_EXISTE");
  if (guardada.estado === "REDIMIDO") {
    const emitido2 = await emiteCredito(guardada, deposito);
    return {
      ok: true,
      yaRedimida: true,
      creditoPendiente: !emitido2,
  ...
  if (cambio === "CONFLICTO") {
    const actual = await deposito.lee(TX, entrada.codigo);
    if (actual?.estado === "REDIMIDO") {
      const emitido2 = await emiteCredito(actual, deposito);
      return {
        ok: true,
        yaRedimida: true,
        creditoPendiente: !emitido2,
  ...
  if (cambio !== "OK") {
    ...
    registra("registrar la redenci\xF3n", new Error(`El almac\xE9n respondi\xF3 ${cambio}.`));
    return {
      ok: false,
      motivo: "ERROR",
...
async function cerrar(codigo, deposito = almacen(), ahora = /* @__PURE__ */ new Date()) {
  const guardada = await deposito.lee(TX, codigo);
  if (!guardada) return rechaza("NO_EXISTE");
  ...
  if (cambio === "CONFLICTO") {
    const actual = await deposito.lee(TX, codigo);
    if (actual?.estado === "REDIMIDO") {
  ...
  if (cambio !== "OK") {
    if (cambio === "NO_EXISTE") return rechaza("NO_EXISTE");
    registra("cerrar la visita", new Error(`El almac\xE9n respondi\xF3 ${cambio}.`));
...
var ALFABETO_CREDITO = "ABCDEFGHJKMNPQRSTUVWXYZ23456789";
function idCredito() {
  const n = new Uint32Array(8);
...
// servidor/redimir.ts
var redimir_default = maneja("POST", async (peticion) => {
  ...
  let personas;
  if (datos.personas !== void 0 && datos.personas !== null) {
    const leidas = entero(datos.personas, 1, MAX_PERSONAS);
    if (leidas.valor === void 0) {
      return {
        estado: 400,
  ...
  if (datos.nota !== void 0 && datos.nota !== null && texto(datos.nota, 200).fallo === "TIPO") {
  ...
  const estado = resultado.ok ? 200 : resultado.motivo === "NO_EXISTE" ? 404 : resultado.motivo === "CONSUMO_INVALIDO" || resultado.motivo === "PERSONAS_INVALIDAS" ? 400 : 409;
  return { estado, cuerpo: resultado };
});
export {
  redimir_default as default
};
